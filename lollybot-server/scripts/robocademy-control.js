...
$(function() {
    ...
    function bindSocketFunctions() {
	    ...
	    for (var i = 0; i < 17; i++) {
		buttonState("#b" + i, data.buttonStates[i]); 
	...
	buttonState("#b18", leftMotorPower > 0);
	buttonState("#b17", rightMotorPower > 0);
    }
    

    // toggle button feedback
    function buttonState(buttonNo, isPressed) {
	if (isPressed) {
	    $(buttonNo).html("on");
	} else {
		$(buttonNo).html("off");
