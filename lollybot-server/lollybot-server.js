...
// DC: always enable logging.
var loggingEnabled = true; // flag for logging
...
if (V_ID == 0 || P_ID == 0 || isNaN(V_ID) || isNaN(P_ID)) {
	...
	// DC: hacked to allow 'Logitech Freedom 2.4' to be recognised as a joystick:
	if ((usbDev.product.toLowerCase().indexOf("joy") != -1 || usbDev.product.toLowerCase().indexOf("logitech") != -1) &&
    ...
    // DC
    console.log("The following joysticks have been detected:");
...
console.log("Setting up hid for: "+V_ID+", "+P_ID);
...
// 3 - debug
//
io.set('log level', 3); // set the Socket.IO logging level to show warnings only
...
function openLogFile() {
    ...
    // DC: Ditch the intricate log file name so we can keep tailing it:
    filename = "log.csv";
